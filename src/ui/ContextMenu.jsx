/**
 * LOOKING GLASS — ContextMenu (Enhanced)
 ...
 *        )}
 *
 *   3. handleContextAction in App.jsx:
 *        const handleContextAction = useCallback((action, item) => {
 *          switch (action) {
 *            case 'open':     window.open(item.content?.url, '_blank'); break;
 *            case 'copy-link': navigator.clipboard.writeText(item.content?.url || ''); break;
 *            case 'stack':    createStack([...selectedIds]); break;
 *            case 'folder':   createFolder([...selectedIds]); break;
 *            case 'summarise': openAISummarise(item); break;
 *            case 'archive':  updateItem(item.id, { meta: { archived: true } }); break;
 *            case 'delete':   deleteItem(item.id); break;
 *            default:
 *              if (action.startsWith('color-')) {
 *                const colors = ['#D71921','#F59E0B','#22C55E','#3B82F6','#8B5CF6'];
 *                const idx    = parseInt(action.split('-')[1]);
 *                updateItem(item.id, { meta: { color: colors[idx] } });
 *              }
 *          }
